...
from gensim.models.phrases2 import Phrases, Phraser, lrsentence, pseudocorpus
...
class TestCommonTermsPhrasesModel(CommonTermsPhrasesData, unittest.TestCase):
    ...
    def testBadParameters(self):
        ...
        # should fail with something less or equal than 0
        self.assertRaises(
            ValueError, Phrases, self.sentences, min_count=0, common_terms=self.common_terms)

        # threshold should be positive
        self.assertRaises(
            ValueError, Phrases, self.sentences, threshold=-1, common_terms=self.common_terms)
