...
class TestPhraseAnalysis(unittest.TestCase):
    ...
    class AnalysisTester(SentenceAnalyzer):
        ...
        def score(self, vocab, word_a, orig_b, bigram_word, min_count):
    ...
    def test_analysis_common_terms_in_between(self):
        ...
        s = ["simple", "sentence", "with", "many", "of", "the", "possible", "bigrams"]
        result = self.analyze_words(scores, s)
        self.assertEqual(
            result, ["simple_sentence", "with", "many_of_the_possible", "bigrams"])

    def test_analysis_common_terms_mix(self):
        scores = {
            b"simple_sentence": 2, b"sentence_with": 2, b"sentence_with_many": 2,
            b"with_many": 2, b"many_possible":2,
            b"many_of": 2, b"the_possible":2, b"many_of_the_possible": 2,
            b"possible_bigrams": 2}

        s = ["the", "simple", "sentence", "with", "many", "of", "the", "possible", "bigrams"]
        result = self.analyze_words(scores, s)
        self.assertEqual(
            result, ["the", "simple_sentence", "with", "many_of_the_possible", "bigrams"])

        s = ["sentence", "with", "so", "many", "of", "possible", "bigrams"]
        result = self.analyze_words(scores, s)
        self.assertEqual(result, ["sentence_with", "so", "many_of", "possible_bigrams"])

        s = ["many", "the", "possible", "bigrams"]
        result = self.analyze_words(scores, s)
        self.assertEqual(result, ["many", "the", "possible_bigrams"])
...
class CommonTermsPhrasesData:
    ...
    sentences = [
        ...
        ['human', 'of', 'time', 'lack', 'of', 'interest'],
